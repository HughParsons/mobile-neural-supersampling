...
class Inpainting(BaseModel):
    def __init__(self, scale_factor: int):
        ...
        self.difference_net = nn.Sequential(
            nn.Conv2d(4, 4, 3, padding=1, stride=1, bias=True),
    ...
    def forward(self, 
        ...
        color_dif = current_color - previous_color[:, :, jitter[0]::2, jitter[1]::2]
        depth_dif = current_depth - previous_depth[:, :, jitter[0]::2, jitter[1]::2]
...
class Blending(BaseModel):
    ...
    def forward(self, current_color: torch.Tensor, previous_color: torch.tensor, jitter: tuple[int, int]) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        previous_color[:, :, jitter[0]::self.scale_factor, jitter[1]::self.scale_factor] = antialiased_color 
